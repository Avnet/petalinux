...
// Used to detect if a device is an IR3806x
// no write operations, only read performed
// version: 0 = IR38060, 2 = IR38062, 3 = IR38063, 4 = IR38064
bool ir3806x_detect ( uint8_t *version )
{
    ASSERT_EMERG(version != NULL, "%s: NULL parameter\n", __FUNCTION__);

    bool ret = false;
    uint8_t reg_addr = 0;
    uint8_t value = 0;

    reg_addr = IC_DEVICE_ID_ADDR;
