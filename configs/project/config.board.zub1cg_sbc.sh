...
case "$PETALINUX_BOARD_PROJECT" in
    ...
    "trifecta")
        ${KCONFIG_EDIT} --file ${CONFIG_FILE} --set-str CONFIG_YOCTO_MACHINE_NAME 'zub1cg-sbc-trifecta';
        ${KCONFIG_EDIT} --file ${CONFIG_FILE} --set-str CONFIG_YOCTO_INCLUDE_MACHINE_NAME "zub1cg-sbc-trifecta";
        ${KCONFIG_EDIT} --file ${CONFIG_FILE} --set-str CONFIG_USER_LAYER_1 '${PROOT}/project-spec/meta-on-semiconductor'
        ;;
